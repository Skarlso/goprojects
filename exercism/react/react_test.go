...
func TestBasicCompute(t *testing.T) {
	...
	if c.Value() != 2 {
		t.Fatalf("c.Value() isn't properly computed based on initial input cell value:%d", c.Value())
	}
	i.SetValue(2)
	if c.Value() != 3 {
		t.Fatalf("c.Value() isn't properly computed based on changed input cell value:%d", c.Value())
	}
}
